...
#   1. Bump the workspace version (Cargo.toml [workspace.package]) + spec
#      Version (+ %changelog) + PKGBUILD pkgver — one commit.
#   2. git tag vX.Y.Z && git push --tags
#   CI does the rest (see README "Install"): the release workflow builds and
#   publishes the Arch package, and COPR rebuilds the SRPM off its GitHub
#   webhook via .copr/Makefile (which runs this script with --head).
#
...
# Workspace version — every member crate inherits it (version.workspace = true).
CARGO_VER=$(sed -n 's/^version = "\(.*\)"/\1/p' "$REPO/Cargo.toml" | head -1)
PKGBUILD_VER=$(sed -n 's/^pkgver=//p' "$REPO/packaging/PKGBUILD")
# Cargo.lock's own entry for the daemon crate (guards a stale lock).
LOCK_VER=$(awk '/^name = "'"$BIN_CRATE"'"$/{getline; gsub(/version = "|"/,""); print; exit}' "$REPO/Cargo.lock")
mismatch=""
